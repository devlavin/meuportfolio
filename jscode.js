    ...
    function draw() {
        ...
        for (let i = 0; i < cols; i++) {
            ...
            if (isInSilhouette(x, y)) {
                ...
                ctx.fillStyle = `rgba(123,104,238,${0.05 + Math.random() * 0.1})`;
            }
...
function mostrarImagem() {
    ...
    document.getElementById('btn-prev').style.display = indexAtual > 0 ? 'flex' : 'none';
    document.getElementById('btn-next').style.display = indexAtual < albumAtual.length - 1 ? 'flex' : 'none';
    document.getElementById('lightbox-contador').textContent = (indexAtual + 1) + ' / ' + albumAtual.length;
}
